$(function() {
	initMySharedFileMenu();
});

/**
 * 初始化右键功能
 */
function initMySharedFileMenu(){
	$.contextMenu({
		selector : "#tb_sharedFileInfo td",
		items : {
			myShared_download : {
				name : "下载",
				callback : function(key, opt) {
					downloadMySharedFile();
				}
			},
			btn_cancelMyShared : {
				name : "取消共享",
 ...
 * 搜索功能
 */
$("#mySharedFileSearch").bind("input propertychange", function(){
	mySharedFileSearch();
});

function mySharedFileSearch(){
	if(isEmpty($.trim($("#mySharedFileSearch").val()))){
		refreshMySharedFileTable();
		return;
	}
	var opt = { 
			url: "../myShared/querySearchInfo.do",
			silent: false, //刷新事件必须设置
			query:{ 
				search : $.trim($("#mySharedFileSearch").val()),
 ...
 * 我的共享文件下载
 */
$("#myShared_download").click(function() {
	downloadMySharedFile();
	//清空rowClick数组
	rowClick.length = 0;
});
 ...
 * 设置下载参数
 */
function createMyShareDownLoadForm(){
	//定义form表单
	var form = $("<form>");
 ...
 */
var mySharedFileFolderIds = new Array();
var mySharedFileFolderNames = new Array();
 ...
 */
function getMySharedLiInfo(folderIds, folderNames) {
	var levelOne = '<li><a href="#" id ="all" class="active"><b>全部文件</b></a></li>';
	$("#mySharedContentOL").html("");
	...
	// 动态生成
	$("#mySharedContentOL").append(levelOne);

	// 绑定事件 (点击目录时候删除目录后面的数据)
	$('#mySharedContentOL li').on('click', function(data) {
		...
		if (liId != "all") {			
			for (var int = 0; int < mySharedFileFolderIds.length; int++) {
			...
			mySharedFileFolderIds = filterFolderIds;
			mySharedFileFolderNames = filterFolderNames;			
			//更新列表信息 
			refreshMySharedFileTable();
		...
		rowClick.length = 0;
		//更新目录结构
		getMySharedLiInfo(filterFolderIds, filterFolderNames);
	});
}
 ...
 * 更新我的共享文件列表
 */
function refreshMySharedFileTable(){
	var currentFolderId = $("#currentFolderId").val();
	if(isEmpty(currentFolderId)){
		$("#tb_sharedFileInfo").bootstrapTable('refresh');
	}else{
		var opt = { 
 ...
 * 点击更新按钮
 */
$('#refreshMySharedTable').click(function(){
	mySharedFileSearch();
});